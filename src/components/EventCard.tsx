...
const EventCard = ({ id, title, date, time, cityName, venueName, categoryName, attendeeCount, description, index = 0, isPast = false, viewMode = "card", isAdmin = false, onEdit, onDelete, onClick, isAuthenticated = true, locationUrl }: EventCardProps) => {
  ...
  return (
    ...
    >
      <Card
        className={`group cursor-pointer overflow-hidden border bg-foreground/5 backdrop-blur-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(0,0,0,0.4)] hover:border-border/20 relative rounded-[2.5rem] h-full transform-gpu ${colors.bg.replace('bg-', 'border-').replace('/15', '/30')}`}
        onClick={handleCardClick}
      >
        {/* Background Orbs */}
        ...
        <CardContent className="p-8 flex flex-col h-full relative z-10">
          ...
          {isAuthenticated && (
            <div className="flex justify-between items-start mb-6">
              <div className="bg-foreground/5 border border-border/20 rounded-2xl p-3 flex flex-col items-center min-w-[60px] shadow-xl">
          ...
          <div className="flex-1">
            {isAuthenticated && (
              <Badge className="mb-4 bg-foreground/5 text-foreground border-border/20 font-bold px-3 py-1 uppercase tracking-wider text-[10px]">
                {categoryName}
              </Badge>
            )}
            ...
            <div className="pt-6 border-t border-border/20 flex items-center justify-between">
              ...
              {isAdmin && (
                <div className="flex items-center gap-2 relative z-50" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
                  <button
                    className="h-10 w-10 rounded-full bg-foreground/5 flex items-center justify-center border border-border/20 hover:bg-amber-500 hover:text-foreground hover:scale-110 active:scale-95 transition-all duration-300 shadow-xl cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <button
                    className="h-10 w-10 rounded-full bg-foreground/5 flex items-center justify-center border border-border/20 hover:bg-red-500 hover:text-foreground hover:scale-110 active:scale-95 transition-all duration-300 shadow-xl cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
                      onDelete?.(id);
                    }}
