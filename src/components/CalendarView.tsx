...
const CalendarView = ({ events, onEventClick, isAuthenticated = true }: CalendarViewProps) => {
  ...
  return (
    <div className="mt-12 space-y-10">
      ...
      >
        <Card className="overflow-hidden border border-border/20 bg-foreground/5 backdrop-blur-lg shadow-[0_32px_64px_rgba(0,0,0,0.4)] rounded-[2.5rem] relative">
          ...
          <CardContent className="p-0 relative z-10">
            ...
            <div className="flex items-center justify-between px-10 py-8 border-b border-border/20 bg-foreground/5">
              <Button
                variant="ghost"
                size="icon"
                onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}
                className="h-12 w-12 rounded-2xl hover:bg-foreground/5 transition-all border border-border/20"
              >
            ...
            {/* Weekday Headers */}
            <div className="grid grid-cols-7 border-b border-border/20 bg-foreground/5 px-2">
              {weekDays.map((day) => (
                <div
                  key={day}
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${!isCurrentMonth ? "opacity-20 pointer-events-none" : "bg-foreground/5 hover:bg-white/[0.08]"}
                      ${isSelected ? "bg-primary/20 border-primary/40 shadow-[0_0_30px_rgba(var(--primary-rgb),0.3)] z-10" : "border-transparent hover:border-border/20"}
                      ${isTodayDate && !isSelected ? "bg-foreground/5 border-primary/50 shadow-[0_0_20px_rgba(var(--primary-rgb),0.2)]" : ""}
                    `}
                  >
                    ...
                    <div className="hidden md:flex flex-col gap-1.5 overflow-hidden flex-1">
                      {dayEvents.slice(0, 2).map((event) => (
                        <div
                          key={event.id}
                          className={`text-[10px] font-bold text-foreground truncate rounded-md px-2 py-1 border-l-2 ${getCategoryDotColor(event.categoryName)} bg-foreground/5 shadow-sm hover:bg-foreground/5 transition-all`}
                        >
                          {event.title}
                        </div>
                      ))}
                    ...
                    {hasEvents && isCurrentMonth && (
                      <div className="md:hidden mt-auto flex justify-end">
                        <div
                          className="h-6 w-6 rounded-full bg-gradient-to-br from-primary to-emerald-400 flex items-center justify-center shadow-[0_0_15px_rgba(var(--primary-rgb),0.3)] border border-border/20 active:scale-95 transition-transform"
                        >
      ...
      <AnimatePresence mode="wait">
        {selectedDate && (
          ...
          >
            <Card className="overflow-hidden border border-border/20 bg-foreground/5 backdrop-blur-lg shadow-2xl rounded-[2.5rem]">
              <CardContent className="p-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-8 border-b border-border/20">
                  ...
                  <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-foreground/5 border border-border/20">
                ...
                {selectedDateEvents.length > 0 ? (
                    ...
                    {selectedDateEvents.map((event, idx) => (
                      <motion.div
                        ...
                        className={`
                          ...
                          border border-border/20 bg-foreground/5 cursor-pointer
                          hover:bg-foreground/5 hover:border-primary/30 transition-all duration-300
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-muted-foreground bg-foreground/5 rounded-[2rem] border border-dashed border-border/20">
