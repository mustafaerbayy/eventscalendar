DROP POLICY IF EXISTS "Budget settings are updatable by authenticated users" ON public.budget_settings;
...
DROP POLICY IF EXISTS "Budget settings are insertable by authenticated users" ON public.budget_settings;
...
CREATE POLICY "Expenses are updatable by creator or managers" ON public.expenses
  FOR UPDATE USING (
...
CREATE POLICY "Expenses are deletable by creator or managers" ON public.expenses
  FOR DELETE USING (
    auth.uid() = created_by OR
    (auth.jwt() ->> 'email') = 'admin@admin.com' OR
    public.has_role(auth.uid(), 'manage_budget')
